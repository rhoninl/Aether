...
**Collision prevention**: UUID v7 encodes a millisecond timestamp in the upper 48 bits and 74 random bits in the remainder, providing strong uniqueness through the combination of temporal ordering and randomness. At 1,000 transactions per second, the probability of a random collision within one millisecond is ~2^-74 (~5×10^-23), well below any practical concern. Note that UUID uniqueness is probabilistic, not guaranteed — the UNIQUE database constraint is the authoritative dedup mechanism. A collision would simply cause an INSERT conflict (treated as a duplicate, rolled back safely), not a double-posting.
...
- WASM scripts have an additional requirement: the platform AOT-compiles scripts at submission time and stores the compiled artifact for server-side execution. On **PC/Desktop**, the client also downloads the raw `.wasm` file for client-side JIT execution and verifies its **SHA-256 content hash** against the approved manifest before loading. On **constrained platforms** (Quest, visionOS, console), user WASM never runs on the client (see Section 8.3) — the server executes user scripts and streams results to the client via normal state sync, so no client-side WASM integrity check is needed for user scripts on these platforms.
